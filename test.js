...
describe('Core Graphic functions', () => {
  ...
  it('should produce an addCurve call for quadratic curves', () => {
    ...
    expect(convertQuadraticCurve(curveData))
      .to.equal(
  ...
  it('should produce an addArc call', () => {
    ...
    expect(convertArc(arcData))
      .to.equal(
        `shape.addArc(withCenter: ${centerCP}, radius: ${radiusCP}, startAngle: 0, endAngle: 360, clockwise: ${Boolean(head(clockwise))})`
      )
  })
  ...
  it('should return various calls based on the data', () => {
    ...
    expect(processPathData(head(parse('L37,17')))).to.include('addLine(')
    expect(processPathData(head(parse('c27,17 37,17 37,17')))).to.include('addCurve(')
    expect(processPathData(head(parse('C27,17 37,17 37,17')))).to.include('addCurve(')
    expect(processPathData(head(parse('q27,17 37,17')))).to.include('addCurve(')
    expect(processPathData(head(parse('Q27,17 37,17')))).to.include('addCurve(')
    expect(processPathData(head(parse('A37,17 37 0,0 37,17')))).to.include('addArc(')
    expect(processPathData(head(parse('Z')))).to.equal(endShape)
    expect(processPathData(['yolo'])).to.equal(`SVG parsing for yolo data isn't supported yet`)
  })
